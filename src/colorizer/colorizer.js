...
function colorize() 
{
    ...
    while(tt = tz.nextToken()) {
      ...
      switch(tt) {
        ...
        case "tag-attr":    if( tagScript && tz.markupAttributeName == "type") tagScriptType = tz.markupAttributeValue; 
                            ...
                            break;
        case "comment":     tz.tokenRange.highlight("tag-comment"); break;
  ...
  doStyle = function(tz, embedded = false) 
  {
    ...
    loop: while(tt = tz.nextToken()) {
      ...
      switch(tt) 
      {
        ...
        case "comment":  tz.tokenRange.highlight("comment"); break;
        case "operator": if ( (tz.tokenValue[0] == ':') && (nmtokenStart != null)) {
          const range = new Range();
          range.setStart(nmtokenStart[0], nmtokenStart[1]);
          range.setEnd(nmtokenStart[0], tz.tokenRange.start[1]);
          range.highlight("property");
          nmtokenStart = null;
        } break;
        case "island-end":
          // got </style>
  ...
  getMethods = (obj) => Object.getOwnPropertyNames(obj).map(item => (typeof obj[item] === 'function') ?  item + "()" : item);
